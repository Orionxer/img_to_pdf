...
1.  Ensure a Python virtual environment (e.g., `.venv` or `venv`) is set up and has `Pillow` installed. Prefer using the virtual environment's interpreter.
2.  Locate the script `img_to_pdf.py` (either in the workspace or within this skill's `scripts` folder).
3.  Run the script with the input image and desired output PDF path using the virtual environment:
    ```bash
    # Assuming venv is in the root
...
3.  Execute the conversion using the project's virtual environment:
    ```bash
    cd skills_example
